<template>
  <div style="background-color: rgb(241, 246, 246);height: 250px;text-align: center;align-items: center;position: relative;margin-top: 50px;">
    <div style="margin: 0 auto;position: absolute;left: 25%;top: 40%;font-size: 35px;">
      <!-- <span>{{msTime.hour}}</span><i>{{hourTxt}}</i> -->
      <span>请在</span>
      <span style="color: red;"> {{msTime.minutes}} </span><i>分</i>
      <span style="color: red;"> {{msTime.seconds}} </span><i>秒</i>
      <span> 内完成支付，超时将自动取消订单</span>
    </div>
  </div>
</template>
<script>
